...
def disambiguate_confirmed(matcher, doc, i, matches):
    ...
    try:
        next_token = doc[i + 1]
        if next_token.text in ["coronavirus", "covid-19", "covid", "sars-cov2"]:
            return
        if next_token.text in ["that"]:
            matches.pop(i)
            return
    except IndexError:
        pass
